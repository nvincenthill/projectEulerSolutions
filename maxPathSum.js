...
// time complexity O(n^2), space complexity O(n) solution
const maxPathSum = triangle => {
  // create current array and initialize to last row of triangle
  let n = triangle.length - 1;
  let current = triangle[n];

  while (n > 0) {
    // access next row above
    let above = triangle[n - 1];
    let temp = [];
    // create new current array with current and above
    for (let i = 0; i < above.length; i += 1) {
      let nextElement = Math.max(current[i], current[i + 1]) + above[i];
      temp.push(nextElement);
    }
    current = temp;
    n -= 1;
  }
